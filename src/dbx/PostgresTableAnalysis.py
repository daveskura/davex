...
class runner():
	...
	def __init__(self,databasetype=dbtype.Postgres,selected_schema='%',selected_table=''):
		...
		if databasetype != dbtype.Postgres:
			print('this unit is for postgres only')
			sys.exit(0)
		...
		for row in data:
			...
			sql = sql.replace('my table comments',col_comment)
			#print(sql)
			#sys.exit(0)
